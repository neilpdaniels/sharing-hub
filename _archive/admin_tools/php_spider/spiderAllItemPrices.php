...
$number_to_spider = 0;
?>
<FORM action="postToSpiderAllItemPrices.php" method="post">
<table width='100%' border='2'>
<?php
foreach ($to_spider as $value)
{
	//if ($number_to_spider!=25) {  ////////////////////////////////////////////
	  //$number_to_spider++;
?>
	<tr>
<?php
	...
	echo "<td width='20%'>Item:". $orderRecord->getorderId()."</td>";
	...
	if ($data!='') {
		if (trim($prePrice)!=''&&trim($postPrice)!='') {
			...
			{
			  if (strlen($price)<70) {
				echo "<td width='70%'>PRICE_CHANGE: from ".$orderRecord->getPrice()." to $price </td>";
				echo "<td width='10%'> <input type='checkbox' name='" . $value . "'
		    	...
		    	echo "<td width='70%'>PRICE_CHANGE: from ".$orderRecord->getPrice()." to TOO_LARGE </td>";
		    	echo "<td>&nbsp;</td>";
  			  }
			} else
			{
				echo "<td width='70%'>NO_PRICE_CHANGE: from ".$orderRecord->getPrice()."</td>";
				echo "<td width='10%'>&nbsp;</td>";			  
			}
			echo "</tr>";	
/*
...
*/
			switch ($orderRecord->getuserId()) {
				case -1:
					if ($price_change) {
							...
							echo "price is < �15 but postage is free<br />";
							echo "<input type='text' size='150' name='" . $value . "_post' value='UPDATE `order` SET `postage` = 3.23, `enteredtime` = NOW( ) WHERE `orderid` = ". $orderRecord->getorderId() ." LIMIT 1'>";
					    	echo "</td></tr>";	
						} elseif ($price>15 && $orderRecord->getPostage()!=0) {
							echo "<tr><td width='100%'>";
							echo "price is > �15 but postage is NOT free<br />";
							echo "<input type='text' size='150' name='" . $value . "_post' value='UPDATE `order` SET `postage` = 0, `enteredtime` = NOW( ) WHERE `orderid` = ". $orderRecord->getorderId() ." LIMIT 1'>";
					    	echo "</td></tr>";							
						}
					} else {
				...
				case -3:
					if ($price_change) {
						if ($price<200 && $orderRecord->getPostage()==0) {
							echo "<tr><td width='100%'>";
							echo "price is < �200 but postage is free<br />";
							echo "<input type='text' size='150' name='" . $value . "_post' value='UPDATE `order` SET `postage` = 5.99, `enteredtime` = NOW( ) WHERE `orderid` = ". $orderRecord->getorderId() ." LIMIT 1'>";
					    	echo "</td></tr>";	
						} elseif ($price>200 && $orderRecord->getPostage()!=0) {
							echo "<tr><td width='100%'>";
							echo "price is > �200 but postage is NOT free<br />";
							echo "<input type='text' size='150' name='" . $value . "_post' value='UPDATE `order` SET `postage` = 0, `enteredtime` = NOW( ) WHERE `orderid` = ". $orderRecord->getorderId() ." LIMIT 1'>";
					    	echo "</td></tr>";							
						}
					} else {
				...
				case -6:
					if ($price_change) {
						if ($price<99 && $orderRecord->getPostage()==0) {
							echo "<tr><td width='100%'>";
							echo "price is < �99 but postage is free<br />";
							echo "<input type='text' size='150' name='" . $value . "_post' value='UPDATE `order` SET `postage` = 3.95, `enteredtime` = NOW( ) WHERE `orderid` = ". $orderRecord->getorderId() ." LIMIT 1'>";
					    	echo "</td></tr>";	
						} elseif ($price>99 && $orderRecord->getPostage()!=0) {
							echo "<tr><td width='100%'>";
							echo "price is > �99 but postage is NOT free<br />";
							echo "<input type='text' size='150' name='" . $value . "_post' value='UPDATE `order` SET `postage` = 0, `enteredtime` = NOW( ) WHERE `orderid` = ". $orderRecord->getorderId() ." LIMIT 1'>";
					    	echo "</td></tr>";							
						}
					} else {
			...
			echo "<td width='80%'>NO_PRICE_SEARCH </td>";
			echo "</tr>";
		}
		...
		if (trim($preQuantity)!=''&&trim($postQuantity)!='') {
			...
			{
			  if (strlen($quantity)<170) {
				echo "<td width='70%'>QUANTITY_CHANGE: from ".$orderRecord->getvolumeasint()." to $quantity</td>";
			  } else {
			    echo "<td width='70%'>QUANTITY_CHANGE: from ".$orderRecord->getvolumeasint()." to TOO_LARGE </td>";
			  }
				echo "<td width='10%'>
				...
				>disable</td>";
			  	...
			  	echo "<td width='70%'>NO_QUANTITY_CHANGE: from ".$orderRecord->getvolumeasint()."</td>";
				echo "<td width='10%'>&nbsp;</td>";		
			}
		} else {
			echo "<td width='80%'>NO_QUANTITY_SEARCH </td>";
		}
		echo "<tr>";
		...
		if (trim($prePostage)!=''&&trim($postPostage)!='') {
			...
			{
			  if (strlen($quantity)<170) {
				echo "<td width='70%'>POSTAGE_CHANGE: from ".$orderRecord->getPostage()." to $postage</td>";
			  } else {
			    echo "<td width='70%'>POSTAGE_CHANGE: from ".$orderRecord->getPostage()." to TOO_LARGE </td>";
			  }
				echo "<td width='10%'> <input type='checkbox' name='" . $value . "'
				value='UPDATE `order` SET `price` =" . $price . ", `enteredtime` = NOW( ) WHERE `orderid` = ". $orderRecord->getorderId() ." LIMIT 1'
				checked >
				change postage
			  	...
			  	echo "<td width='70%'>NO_POSTAGE_CHANGE: from ".$orderRecord->getPostage()."</td>";
				echo "<td width='10%'>&nbsp;</td>";		
			}
		} else {
			echo "<td width='80%'>NO_POSTAGE_SEARCH </td>";
		}
		echo "<tr>";

	echo "<tr><td colspan='3'>URL: <a href='" . $this_spider->url . "'>" . $this_spider->url . "</a></td></tr>";	
	} else {
	echo "	<tr><td colspan='3'>URL: NO_WEBSITE</td></tr>";	
	}
	echo "<tr><td colspan='3' BGCOLOR='#ffff00'>&nbsp;</td></tr>";
	sleep (2);
	}
	//}	////////////////////////////////////////////
?>
