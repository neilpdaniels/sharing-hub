...
function postActivatePassword(url, parameters) {
	var centerCont = document.getElementById("activate_status");
    if(centerCont){
      centerCont.innerHTML="<p>Checking...</p>";
    }
	postDivContent(centerCont, url, parameters);
...
function passwordCheck(userid, password, url) {
  var conPass = document.getElementById("validPassword");
  if(conPass){
    conPass.innerHTML="<div><b>Checking password...</b></div>";
  }
  parameters  = "userId=" + userid + "&password=" + hex_md5(password);
  postDivContent(conPass, url, parameters);
}
...
function signIn(username, password, root) {
  ...
  if(signInCont){
    signInCont.innerHTML="<div>Processing...</div>";
  }
  var XMLHttpRequestObject = createXMLrequest();
  if(XMLHttpRequestObject) {
	XMLHttpRequestObject.open("POST", url);
...
function signOut(root) {
  var signInCont = document.getElementById("signOut");
...
function showGrid($counter, $orderId) {
  ...
  if (wrapperCont.style.display == "none") {
	  ...
	  increaseGridSize();
          removejscssfile('../../js/templt/jquery.js','js');
          removejscssfile('jquery.js','js');
          removejscssfile('../../js/templt/opentip-jquery.js','js');
          removejscssfile('opentip-jquery.js','js');
          var fileref=document.createElement('script');
          fileref.setAttribute("type","text/javascript");
          fileref.setAttribute("src", "../../js/templt/jquery.js");
          document.getElementsByTagName("head")[0].appendChild(fileref);

          var fileref2=document.createElement('script');
          fileref2.setAttribute("type","text/javascript");
          fileref2.setAttribute("src", "../../js/opentip-jquery.js");
