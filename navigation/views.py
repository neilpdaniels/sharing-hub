    ...
    if distance_filter == 0:
        ...
        for product in products:
            ...
            for order in product.order_set.filter(status=Order.ACTIVE, latitude__isnull=False):
                d = _G.calculate_distance(browse_lat, browse_lon, float(order.latitude), float(order.longitude))
    ...
    if max_distance_km == 0:
        ...
        for order in orders:
            if order.latitude and order.longitude:
                dist = _G.calculate_distance(latitude, longitude, order.latitude, order.longitude)
                order.distance = dist
        ...
        def _within(qs):
            ...
            for o in qs:
                if o.latitude and o.longitude:
                    d = _G.calculate_distance(page_lat, page_lon, float(o.latitude), float(o.longitude))
                    o.distance_km = d
...
def howItWorks(request):
    allCategories = Category.objects.all()
    template = loader.get_template('navigation/how_it_works.html')
    # output = ', '.join([q.title for q in allCategories])
    context = {
        'allCategories' : allCategories
    }
...
def helpAndSupport(request):
    template = loader.get_template('navigation/help_and_support.html')
    context = {}
    return HttpResponse(template.render(context, request))

def buyersGuide(request):
    template = loader.get_template('navigation/information_for_buyers.html')
    context = {}
    return HttpResponse(template.render(context, request))

def sellersGuide(request):
    template = loader.get_template('navigation/information_for_sellers.html')
    context = {}
    return HttpResponse(template.render(context, request))
    
def transactionGuide(request):
    template = loader.get_template('navigation/transaction_workflow.html')
    context = {}
    return HttpResponse(template.render(context, request))

def physicalOwnershipGuide(request):
    template = loader.get_template('navigation/physical_ownership.html')
    context = {}
    return HttpResponse(template.render(context, request))
    
def siteFeedback(request):
    template = loader.get_template('navigation/site_feedback.html')
    context = {}
    return HttpResponse(template.render(context, request))

def aboutUs(request):
    template = loader.get_template('navigation/about_us.html')
    context = {}
    return HttpResponse(template.render(context, request))

def termsAndConditions(request):
    template = loader.get_template('navigation/terms_and_conditions.html')
    context = {}
    return HttpResponse(template.render(context, request))
