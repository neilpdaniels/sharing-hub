...
class _RegisterScreenState extends State<RegisterScreen> {
  static const Set<String> _blockedUsernameWords = {
    'asshole',
    'bastard',
    'bitch',
    'bullshit',
  ...
  void _scheduleUsernameCheck() {
    ...
    if (_isBlockedUsername(username)) {
      setState(() {
    ...
    setState(() {
      ...
      _usernameAvailable = null;
      _usernameHint = 'Checking availability...';
    });
    ...
    _usernameDebounce = Timer(const Duration(milliseconds: 350), () async {
      ...
      try {
        final result = await widget.authRepository.checkUsername(username);
        if (!mounted) {
          return;
        }
        ...
        setState(() {
          _usernameAvailable = result.$1;
          _usernameHint = result.$1
              ? 'Username available'
        ...
        setState(() {
          _usernameHint = 'Could not check username right now';
          _usernameAvailable = null;
          _checkingUsername = false;
        });
      }
  ...
  bool _isBlockedUsername(String username) {
    ...
    return _blockedUsernameWords.any((word) {
      if (tokens.contains(word)) {
        return true;
      }
      return word.length >= 4 &&
          (normalized.contains(word) || compact.contains(word));
    });
  }
