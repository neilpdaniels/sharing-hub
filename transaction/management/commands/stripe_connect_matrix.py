...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--include-declines', action='store_true', help='Also verify generic and insufficient-funds decline paths.')
    ...
    def handle(self, *args, **options):
        ...
        if options['include_declines']:
            for name, token in DECLINE_CASES:
                try:
                    ...
                    if code != 'card_declined':
                        raise CommandError(f'{name}: expected card_declined, got {code or type(exc).__name__}.') from exc
                    self.stdout.write(self.style.SUCCESS(f'PASS {name}: card_declined'))
                else:
